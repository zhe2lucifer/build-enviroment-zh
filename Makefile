...
BBLAYERS ?= \
	...
	$(CURDIR)/meta-oe-alliance/meta-brands/meta-edision2 \
	...
	$(CURDIR)/meta-oe-alliance/meta-brands/meta-torch \
...
$(TOPDIR)/env.source: $(DEPDIR)/.env.source.$(BITBAKE_ENV_HASH)
	...
	@echo ' fi' >> $@
	@echo 'fi' >> $@
	@test -d $(TOPDIR)/cache || mkdir -p $(TOPDIR)/cache
	@ln -s $(CURDIR)/bb_persist_data.sqlite3 $(TOPDIR)/cache/
